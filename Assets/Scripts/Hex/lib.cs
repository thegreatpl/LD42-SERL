...
struct Hex
{
    ...
    {
        if (direction < 0 || direction > directions.Count)
            return new Hex(0, 0); 
...
struct OffsetCoord
{
    ...
    static public OffsetCoord QoffsetFromCube( Hex h, int offset)
    {
        int col = h.q;
    ...
    static public OffsetCoord RoffsetFromCube( Hex h, int offset = -1)
    {
    ...
    static public Hex RoffsetToCube( OffsetCoord h, int offset = -1)
    {
    ...
    static public Vector3Int RToUnityCoords(Hex h, int offset = -1)
    {
        int col = h.q + (int)((h.r + offset * (h.r & 1)) / 2);
        int row = h.r;
...
//    }
    ...
    //    Tests.EqualHex("hex_round 1", new Hex(5, -10, 5), new FractionalHex(0, 0, 0).HexLerp(new FractionalHex(10, -20, 10), 0.5).HexRound());
